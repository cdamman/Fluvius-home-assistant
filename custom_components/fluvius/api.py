...
class FluviusApiClient:
    ...
    def _build_gas_day_range(self, days_back: int) -> dict[str, str]:
        ...
        Fluvius answers HTTP 200 with an empty list for any gas window that does not
        line up with the gas day, so unlike electricity the range is never clamped to
        a cutoff falling inside one: the anchor steps back to the last gas day that
        had closed by then.
        """
